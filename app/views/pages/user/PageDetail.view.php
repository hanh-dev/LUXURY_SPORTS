			...
			<div class="content-2">
				...
				<div class="products">
					...
					<?php foreach($data['RelatedProducts'] as $relatedProduct): ?>
						<?php $isFavoriteRelated = $relatedProduct['isFavorite']; ?>
						<a href="Details/show/<?= $relatedProduct['ID'] ?>">
							<div class="product-item">
								<div class="wrapp_heart">
									<i class="<?= $relatedProduct['isFavorite'] ? 'fa-solid fa-heart' : 'fa-regular fa-heart' ?>" 
										data-id="<?= $relatedProduct['ID'] ?>"
										onclick="<?= $relatedProduct['isFavorite'] ? "removeProductFromWishList({$relatedProduct['ID']})" : "addToWishList({$relatedProduct['ID']})" ?>">
									</i>
								</div>
